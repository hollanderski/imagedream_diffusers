...
class MVDreamLoRATrainer:
    ...
    def setup_training(self):
        ...
        resume = False
        if resume: 
            ckpt = r"G:/ninon_workspace/imagedream_diffusers/eeg_mvdream_lora_8v_scaled_P12/checkpoint-5000"
    ...
    def compute_loss(self, batch, current_step=0, max_steps=1):
        ...
        # EEG embeddings (ventral stream features)
        ...
        # current_norm = eeg_embeddings_expanded.norm(dim=-1).mean()
        # target_norm = 100.0  #
        # scaling_factor = target_norm / (current_norm.item() + 1e-8)
        # eeg_embeddings_expanded_scaled = eeg_embeddings_expanded * scaling_factor
        # # [B, num_views, 77, 1024]
        # # Flatten for UNet input
        # eeg_embeddings_flat = eeg_embeddings_expanded_scaled.view(-1, 77, 1024)


        # print("BEFORE FIX")
        # for i in range(min(10, eeg_embeddings_flat.shape[0])):
        #     b = i // num_views
        #     v = i % num_views
        #     context_class = batch['classes'][b].item()
        #     print(f"Flat idx {i}: batch {b}, view {v}, EEG class {context_class}")
        #     eeg_vec = eeg_embeddings_flat[i]
        #     print(f"Sample {i}: context mean {eeg_vec.mean().item():.4f}")
        ...
        target_norm = 165.0 #165.0
        scaling_factor = target_norm / (current_norm.item() + 1e-8)
        eeg_context_token_scaled = eeg_context_token * scaling_factor
        eeg_context_flat = eeg_context_token_scaled.view(-1, 77, 1024) 

        #eeg_context_flat = eeg_context_token.view(-1, 77, 1024)                         # [B*V, 77, 1024]

        # for i in range(min(10, eeg_context_flat.shape[0])):
        #     b = i // num_views
        #     v = i % num_views
        #     context_class = batch['classes'][b].item()
        #     print(f"Flat idx {i}: batch {b}, view {v}, EEG class {context_class}")
        #     eeg_vec = eeg_context_flat[i]
        #     print(f"Sample {i}: context mean {eeg_vec.mean().item():.4f}")

        # all_equal = True
        # first_vec = eeg_context_flat[0].cpu().detach().numpy()
        # for i in range(1, eeg_context_flat.shape[0]):
        #     if not np.allclose(first_vec, eeg_context_flat[i].cpu().detach().numpy()):
        #         all_equal = False
        #         break
        # print("Are all EEG contexts identical?", all_equal)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./eeg_mvdream_lora_8v_scaled165_P12", help="Output directory")
